...
function MenuMapEditor:game_loaded()
	managers.hud:add_updator("UnitHighlighter", callback(self, self, "UnitHighlighter"))
end
function MenuMapEditor:HideOptions()
...
function MenuMapEditor:ShowOptions()
...
function MenuMapEditor:key_press( o, k )
	...
	local optionskey = "f9"
	if self._menu_closed and k == Idstring(optionskey) then
		self:ShowOptions()
	elseif not self._menu_closed and (k == Idstring(optionskey)) then
		self:HideOptions()
...
function MenuMapEditor:CreateItems()
    ...
    self:CreateItem({
        ...
        help = "",
        value = false,
        callback = "set_editor_units_visible",
        parent = "continents_options",
        type = "toggle"  
    })      
    self:CreateItem({
...
function MenuMapEditor:UnitHighlighter()		
	...
	if self:get_item("units_highlight").value then
		...
		if managers.viewport:get_current_camera() then
			...
			if alive(self._selected_unit) then
				name_brush:center_text(self._selected_unit:position() + Vector3(-10, -10, 200), self._selected_unit:unit_data().name_id .. "[ " .. self._selected_unit:editor_id() .. " ]", cam_right, -cam_up)
			end
		end	
	end
end
...
function MenuMapEditor:set_editor_units_visible( item )
	for _, unit in pairs(World:find_units_quick("all")) do
...
function MenuMapEditor:set_unit( unit )
	...
	self:set_value(self:get_item("unit_id"), unit and unit:editor_id() or "")	
...
function MenuMapEditor:set_unit_data( item )
	if alive(self._selected_unit) then
		local FreeFlight = setup:freeflight()
		FreeFlight:set_position(Vector3(self:get_item("positionx").value, self:get_item("positiony").value, self:get_item("positionz").value), Rotation(self:get_item("rotationyaw").value, self:get_item("rotationpitch").value, self:get_item("rotationroll").value))
