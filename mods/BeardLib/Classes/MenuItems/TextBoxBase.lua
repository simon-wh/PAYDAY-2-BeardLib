...
function TextBoxBase:init(params)
    ...
    local caret = params.panel:rect({
        ...
        w = 1,
        visible = false,
        color = (self.text_color / 1.2):with_alpha(1),
        h = bg:h() - 2,
        layer = 12,
    })          
...
function TextBoxBase:update_caret()		
	local text = self.text_panel:child("text")    
    local bg = self.text_panel:child("textbg")
	...
	local lines = math.max(1,text:number_of_lines()) 
 	...
 	if self.type == "TextBox" then	
		...
		text:set_h(bg:h())
 		self.text_panel:set_h((self.items_size * (lines + 1)))  
	    bg:set_world_bottom(self.text_panel:world_bottom())
		if self.group then
...
function TextBoxBase:MousePressed( button, x, y )
    ...
    self.cantype = text:inside(x,y) and button == Idstring("0")
