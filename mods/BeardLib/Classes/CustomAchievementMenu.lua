...
function CustomAchievementMenu:init()
    ...
    MenuCallbackHandler.CustomAchievementMenu = ClassClbk(self, "_show", true)
    MenuHelperPlus:AddButton({
        id = "CustomAchievementMenu",
        title = "beardlib_customachievementmenu_title",
        desc = "beardlib_customachievementmenu_desc",
        callback = "CustomAchievementMenu",
        menu = "menu_main",
        position = 58   -- Probably able to give something more clean than a random number? Hell if I know.
    })
...
function CustomAchievementMenu:_init_header()
    local icon_trophy = self._header:ImageButton({ texture = "guis/textures/achievement_trophy_white", h = 30, w = 30, highlight_color = Color.transparent })
...
function CustomAchievementMenu:_init_account()
    ...
    local steam_avatar = panel:ImageButton({ texture = "guis/texture/pd2/none_icon", w = 64, h = 64 })
...
function CustomAchievementMenu:_init_packages()
    ...
    for _, package_data in pairs(sorted_packages) do
        ...
        local package_icon = panel:ImageButton({
...
function CustomAchievementMenu:_display_achievement_details(achievement)
    ...
    local achiev_icon = panel:ImageButton({
    ...
    if achievement:_has_reward() then
        ...
        if achievement:_get_reward_type() == "xp" then
            local reward_icon = achiev_reward_panel:ImageButton({
                texture = xp_icon,
                h = 64,
                w = 64
            })
